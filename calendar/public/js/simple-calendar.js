...
var SimpleCalendar = function () {
  ...
  _createClass(SimpleCalendar, [{
    ...
    value: function addData(year, month) {
      ...
      var need_date = new Array

      $.ajax({
            url: 'index.php?r=calendar/get_sgin_all',
            type: 'get',
            dataType:'json',
            success:function(daa){
              $.each(daa, function(k, v) {
                 need_date[k] = parseInt(v.date)
      ...
      for (var i = 0; i < 35; i++) {
        ...
        var now_date = writeyear+'-'+writemonth+'-'+writeday
        var timestamp = Date.parse(new Date(now_date))/1000;

        if ($.inArray(timestamp, need_date) > 0) {
